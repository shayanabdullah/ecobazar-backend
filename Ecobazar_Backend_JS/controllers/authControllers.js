...
const sendOtpController = async (req, res) => {
    try {
        ...
        existingUser.verifyOtpExpire = new Date(Date.now() + 10 * 60 * 1000);
        await existingUser.save();
        await sendVerificationEmail(existingUser.email, existingUser.fullName, otp);
        return res.status(200).json({
            success: true,
...
const verifyOtpController = async (req, res) => {
    try {
        ...
        return res.status(500).json({
            success: false,
            message: "Unable to verify your account at this time. Please try again later.",
        });
    }
};
