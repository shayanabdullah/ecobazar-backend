...
export const sendVerificationEmail = async (email, name, otp) => {
    try {
        ...
        console.log("Verification OTP email sent:", response.messageId);
        return response;
    }
    catch (error) {
...
// temporary
export const sendEmailToAdminForCreateCategory = async (email, name, categoryId, categoryName) => {
    const approveUrl = `${process.env.BACKEND_URL}/api/v1/admin/category/approve/${categoryId}`;
    const rejectUrl = `${process.env.BACKEND_URL}/api/v1/admin/category/reject/${categoryId}`;
    try {
        const response = await brevo.transactionalEmails.sendTransacEmail({
            sender: {
                name: process.env.BREVO_SENDER_NAME,
